...
- `slides/01.svg` … `slides/19.svg` — one SVG per slide, each self-contained
  ...
  per mm). Nothing on a slide is smaller than ~11 pt in print.
- `index.html` — the deck: arrow keys, click zones, swipe, `o` for the
  overview grid, `f` for fullscreen, deep links (`#7`), and a print
  stylesheet that emits one A5 landscape page per slide.
...
| 4 | The method: automated reasoning |
